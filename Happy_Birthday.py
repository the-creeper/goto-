...
@media screen and (min-width: 670px) {
  ...
  h2 {
    ...
    top: 50px;
    font-size: 90px;
    background: -webkit-linear-gradient(90deg, #e9e6ff 0%, red 100%);
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
  }
...
'''
    with open('生日快乐.html','w',encoding='utf-8') as fs:
        fs.write(html1 + text + html2 + html3 + text + html4 + html5)
    with open('生日快乐.css','w',encoding='utf-8') as fs:
        fs.write(css)
    os.system('生日快乐.html')
    time.sleep(0.5)
    os.remove('生日快乐.html')
